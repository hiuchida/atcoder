...
//				System.out.println(pw + " " + info + " " + s);
				if (s > 0 && s < (double)pw / xx) {
					pw += s;
					map[y][x] = 0;
					if (y > 1 && map[y-1][x] > 0) que2.offer(new Info(y-1,x));
					if (y < h && map[y+1][x] > 0) que2.offer(new Info(y+1,x));
					if (x > 1 && map[y][x-1] > 0) que2.offer(new Info(y,x-1));
					if (x < w && map[y][x+1] > 0) que2.offer(new Info(y,x+1));
					bUpdate = true;
				} else {
					que2.offer(info);
				}
