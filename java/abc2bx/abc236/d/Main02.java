...
//		if (idx==6) System.out.println(idx+" "+val+" "+Arrays.toString(flag));
		if (idx==2*n) {
		...
		for (int i=1; i<=2*n-1; i++) {
			...
			if (idx==4 && i>5) continue;
			if (idx==6 && i>7) continue;
			if (idx==8 && i>9) continue;
			if (idx==10 && i>11) continue;
			if (idx==12 && i>13) continue;
			if (idx==14 && i>15) continue;
			if (flag[i]) continue;
			flag[i]=true;
			for (int j=i+1; j<=2*n; j++) {
				if (flag[j]) continue;
