...
public class Main {
	...
	public static void main(String[] args) {
		...
		init(sc);
		Point[] bap=new Point[m];
		System.arraycopy(ap, 0, bap, 0, m);
		btn=new Button();
		btn.print();
...
//			System.out.println("lines="+ls.size());
			if (ls.size()<ansl) {
				ansl=ls.size();
				ans=ls;
			}
	...
	static int calc(int idx, char ch) {
		Point pt=ap[idx];
		Point pn=mw.move(pt, ch);
		if (pt.equals(pn)) return 0;
		if (!map[pn.y][pn.x]) return 1;
		return 0;
	}
	static Point[] makeTarget() {
		...
		for (int y0=0; y0<n; y0++) {
			for (int j=0; j<n; j++) {
				int x0=(y0%2==0) ? j : (n-1-j);
				ans[idx++]=new Point(y0, x0);
			}
		}
		return ans;
	}
	static Point[] makeTarget2() {
		int[][] flag=bfsAll();
		Point[] ans=new Point[n*n];
...
//		System.out.println(sb);
	...
	static class MazeWall {
		...
		Point move(Point p, char d) {
			if (!mw.canMove(p, d)) return p;
			int idx=getIdx(d);
			if (idx<0) return p;
			p=new Point(p.y+DY[idx], p.x+DX[idx]);
			return p;
		}
	}
