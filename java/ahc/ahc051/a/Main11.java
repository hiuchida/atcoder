...
//					System.out.println("debug "+ll.get(i)+" "+ll.get(j)+" "+i+" "+j);
	...
	boolean isCross(List<Line> ll, Line that) {
		for (int i=0; i<ll.size(); i++) {
			if (ll.get(i).isIntersect(that)) {
//				System.out.println("debug "+ll.get(i)+" "+that+" "+i);
				return true;
			}
		}
		return false;
...
//		}
		...
		while (true) {
			if (isTimeout()) break;
			List<Line> ll=new ArrayList<>();
			int p01=rand.nextInt(m);
			int p11=rand.nextInt(m);
			int p12=rand.nextInt(m);
			if (p01==p11 || p01==p12 || p11==p12) continue;
			ll.add(new Line(og, aps[p01]));
			ll.add(new Line(aps[p01], aps[p11]));
			ll.add(new Line(aps[p01], aps[p12]));
			while (true) {
				if (isTimeout()) break;
				int a11=rand.nextInt(n);
				int b11=rand.nextInt(n);
				int a12=rand.nextInt(n);
				int b12=rand.nextInt(n);
				if (a11==a12 || a11==b11 || a11==b12) continue;
				if (a12==b11 || a12==b12 || b11==b12) continue;
				Line l111=new Line(aps[p11], apd[a11]);
				Line l112=new Line(aps[p11], apd[b11]);
				Line l211=new Line(aps[p12], apd[a12]);
				Line l212=new Line(aps[p12], apd[b12]);
				if (isCross(ll, l111)) continue;
				ll.add(l111);
				if (isCross(ll, l112)) continue;
				ll.add(l112);
				if (isCross(ll, l211)) continue;
				ll.add(l211);
				if (isCross(ll, l212)) continue;
				while (true) {
					if (isTimeout()) break;
					int k01=rand.nextInt(k);
					int k11=rand.nextInt(k);
					int k12=rand.nextInt(k);
					if (k01==k11 || k01==k12 || k11==k12) continue;
					double v0111=add[k01][a11];
					double v1111=add[k11][a11];
					double v0112=add[k01][b11];
					double v1112=1-add[k11][b11];
					double v0121=1-add[k01][a12];
					double v1211=add[k12][a12];
					double v0122=1-add[k01][b12];
					double v1212=1-add[k12][b12];
					double v=v0111*v1111+v0112*v1112+v0121*v1211+v0122*v1212;
					if (maxv<v) {
						maxv=v;
						p01i=p01;
						p11i=p11;
						...
						a12i=a12;
						b12i=b12;
						System.err.println("iteration: " + iteration + ", maxv: " + maxv+" A:"+k01+" "+k11+" "+k12);
						break;
					}
					iteration++;
				}
...
//		}
		...
		ans.s=n+p01i;
		for (int i=0; i<m; i++) {
			if (i==p01i) ans.as[i]=new Sorter(k01i, n+p11i, n+p12i);
			else if (i==p11i) ans.as[i]=new Sorter(k11i, a11i, b11i);
			else if (i==p12i) ans.as[i]=new Sorter(k12i, a12i, b12i);
