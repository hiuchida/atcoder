...
public class Main {
	...
	boolean checkV(int x) {
		for (int y=0; y<n; y++) {
			if (map[y][x]!=0) return true;
		}
		return false;
	}
	boolean checkH(int y) {
		for (int x=0; x<n; x++) {
			if (map[y][x]!=0) return true;
		}
		return false;
	}
	void clearU(int p, int k) {
		for (int y=0; y<k; y++) {
			map[y][p]=0;
		}
	}
	void clearD(int p, int k) {
		for (int y=0; y<k; y++) {
			map[n-1-y][p]=0;
		}
	}
	void clearL(int p, int k) {
		for (int x=0; x<k; x++) {
			map[p][x]=0;
		}
	}
	void clearR(int p, int k) {
		for (int x=0; x<k; x++) {
			map[p][n-1-x]=0;
		}
	}
	void move(char d, int p, int k) {
		...
		case 'U':
			clearU(p, k);
			if (checkV(p)) move1('D', p, k);
			break;
		case 'D':
			clearD(p, k);
			if (checkV(p)) move1('U', p, k);
			break;
		case 'L':
			clearL(p, k);
			if (checkH(p)) move1('R', p, k);
			break;
		case 'R':
			clearR(p, k);
			if (checkH(p)) move1('L', p, k);
			break;
		}
	}
