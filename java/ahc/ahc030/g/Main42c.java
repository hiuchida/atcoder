...
//                	System.err.println("DEBUG get_ln_pR_if_x "+size+" "+failed_coordinates.length);
    ...
    static class Query {
        ...
        Query(Input in, List<OilLayout> pool) {
            this.in_query = new boolean[in.n2];
        ...
        double eval(Sim sim) {
            ...
            double cost = 1.0 / Math.sqrt(coordinate_size);
            List<Double> prR = new ArrayList<>();
            for (int x = 0; x < pool.size(); x++) {
                int v = volume[x];
//                System.err.println("DEBUG x="+x+" v="+v+" coordinate_size="+coordinate_size+" "+Arrays.toString(poolVol));
                int lb = sim.pr_if_x_lb[coordinate_size][v];
                List<double[]> probs = sim.pr_if_x.get(coordinate_size).get(v);
                while (prR.size() < lb + probs.size()) {
                	prR.add(0.0);
                }
...
//            	l.lnPRifX = sim.lnProbState(
//            			new State(input){{ topLefts=l.topLefts; volumes=l.volume; queryVolumes=state.queryVolumes; }});
//            }
            ...
            if (t == 0) {
                ...
                // 焼きなましによるプール補充
                performSA(input, sim, state, pool, seen, ITER, startTime);
            }

            double maxLn = pool.get(0).ln_pR_if_x;
            double sumExp = 0;
            for (OilLayout l : pool) {
            	l.px_if_R = Math.exp(l.ln_pR_if_x - maxLn);
            	sumExp += l.px_if_R;
            }
            for (OilLayout l : pool) {
            	l.px_if_R /= sumExp;
...
//                for(int i=0; i<in.n2; i++) {
//                	if(qh.in_query[i]) qCoords.add(i);
//                }
    ...
    static void performSA(Input in, Sim sim, State state, List<OilLayout> pool, Map<Long, Double> seen, int ITER, long start) {
        ...
        for (int i=0; i<in.m; i++) {
        	state.moveTo(i, pool.get(0).top_lefts[i]);
        }
        ...
        for (int i = 0; i < saIter; i++) {
            ...
            if (!seen.containsKey(state.hash)) {
                ...
                if (nextL - maxL >= -10.0) {
                	pool.add(new OilLayout(state.hash, nextL, 0.0, state.top_lefts, state.volumes.clone()));
                }
            }
            if (nextL >= currentL || rng.gen_bool(Math.exp((nextL - currentL)/temp))) {
            	currentL = nextL;
            } else {
            	state.moveTo(id, oldTL);
            }
            if (currentL > maxL) {
            	maxL = currentL;
            }
