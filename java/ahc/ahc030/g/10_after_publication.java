    ...
    static void simulatedAnnealing(Input in, Sim sim, State s, int[][][] swaps, Map<Long, Double> hL, int iter, long startT) {
        ...
        for (int t = 0; t < iter; t++) {
            ...
            if (coin < 30) { // Slide
                ...
                int ni = bk1 / in.n + d, nj = bk1 % in.n + d;
                ...
                int ni1 = (bk2 / in.n) + d1, nj1 = (bk2 % in.n) + d1;
                int ni2 = (bk1 / in.n) + d2, nj2 = (bk1 % in.n) + d2;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < input.m; i++) {
            ...
            for (int j = 0; j < size; j++) {
                os.maxI = Math.max(os.maxI, os.coordinates.get(j)); os.maxJ = Math.max(os.maxJ, os.coordinates.get(j));
                os.coordinateIds[j] = os.coordinates.get(j) * input.n + os.coordinates.get(j); os.mask.set(os.coordinateIds[j]);
            }
        }
        ...
        for (int a = 0; a < input.m; a++) {
            for (int b = 0; b < input.m; b++) {
                ...
                for (int i = 0; i < sz; i++) swaps[a][b][i] = new int[]{list.get(i), list.get(i)};
            }
        }
        ...
        for (int t = 0; ; t++) {
            ...
            if (bestProb > 0.8) {
                ...
                if (!input.isDifficult() || (sim.queries.size() > 0 && Arrays.equals(sim.queries.get(sim.queries.size()-1).first.stream().mapToInt(x->x).toArray(), revT.stream().mapToInt(x->x).toArray()))) {
                    if (sim.ans(T)) break;
                } else {
