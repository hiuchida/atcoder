...
//    static final int[] DC = {1, 0, -1, 0};
    ...
    static class State {
        ...
        State(Input in) {
            ...
            this.hash = 0;
            for (int i = 0; i < in.m; i++) this.hash ^= oilStates[i].hashes[0];
    ...
    static class Sim {
        ...
        void giveup(Scanner sc) {
            ...
            que.add(new Point(input.n / 2, input.n / 2));
            ...
            while (!que.isEmpty()) {
                ...
                if (ret > 0) {
                    found.add(p.r * input.n + p.c);
                    remT -= ret; if (remT <= 0) break;
                }
                for (int[] d : DIJ) {
                	int nr = p.r + d[0], nc = p.c + d[1];
                    if (nr >= 0 && nr < input.n && nc >= 0 && nc < input.n && !used[nr][nc]) {
