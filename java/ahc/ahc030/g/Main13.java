...
//			return rand.nextBoolean();
	...
	static class OilLayout implements Comparable<OilLayout> {
		...
		BitSet mask;
		OilLayout(double ln, double px, int[] tls, int[] vol, BitSet mask) {
			this.ln_pR_if_x = ln;
			this.px_if_R = px;
			...
			for (int j_a = 0; j_a < input.n - oil_a.max_j; j_a++) {
				...
				for (int i_b = 0; i_b < input.n - oil_b.max_i; i_b++) {
					for (int j_b = 0; j_b < input.n - oil_b.max_j; j_b++) {
						...
						BitSet mask=input.get_positives(state.top_lefts);
						pool.add(new OilLayout(0.0, 0.0, state.top_lefts, state.volumes, mask));
//						int key = mask.hashCode();
...
//		System.err.println(pool.size()+" "+cnt.size());
//		for (int key : cnt.keySet()) {
//			if (cnt.get(key)>1) System.err.println(key+" "+cnt.get(key));
//		}
...
//				}
				...
				if (sim.ans(T_vec)) {
					...
					List<OilLayout> copy = new ArrayList<>();
					for (OilLayout ol : pool) {
						if (!best_bits.equals(ol.mask)) copy.add(ol); 
					}
					pool = copy;
//					if (sim.failed.size() == 1) {
