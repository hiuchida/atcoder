...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            // 1. 焼きなましによるプール更新 (近傍の工夫版)
            pool = updatePoolWithSA(current, history);
    ...
    static List<Configuration> updatePoolWithSA(Configuration start, List<QueryRecord> history) {
        ...
        for (int i = 0; i < iters; i++) {
            ...
            if (newL > oldL || rand.nextDouble() < Math.exp((newL - oldL) / temp)) {
                ...
                // 却下時の差し戻し（入れ替えの場合は両方戻す必要あり）
                cur.moveOil(oilId, oldTL, history);
            }
        }
        Collections.sort(pool);
        int poolLimit = (System.currentTimeMillis() - startTime > 2000) ? 100 : 500; // 時間経過で絞る
        return pool.subList(0, Math.min(pool.size(), poolLimit));
    }
