    ...
    public static Output solve(final Input input) {
        ...
        for (int i = 0; i < input.pickup_count; ++i) {
            ...
            for (int j = 0; j < input.order_count; ++j) {
                // 【穴埋め】既に訪れていたらスキップ
                if (visited_restaurant.get(j)) continue;
    
                // 【穴埋め】最短距離が更新されたら記録
                // 【ヒント】int distance = p0.dist(p1); と書くと、p0とp1のマンハッタン距離が計算できる
                // 【ヒント】nearest_restaurant, min_distの2つを更新する
                if (min_dist>current_position.dist(input.restaurants.get(j))) {
                	min_dist=current_position.dist(input.restaurants.get(j));
        ...
        for (int i = 0; i < input.pickup_count; ++i) {
            ...
            for (int j = 0; j < destinations.size(); ++j) {
                ...
                if (min_dist>current_position.dist(input.destinations.get(destinations.get(j)))) {
                	min_dist=current_position.dist(input.destinations.get(destinations.get(j)));
                	nearest_index=j;
