...
//		System.out.println(Arrays.toString(ar));
		for (int i=0; i<N; i++) {
			int v1=calcL1(og, ar[i].st);
			int v2=calcL1(og, ar[i].ed);
...
//            double progress = (double) Duration.between(start_time, current_time).toMillis() / time_limit;
	...
	static int minvsted(Point p0) {
		...
		for (int i=0; i<N; i++) {
			if (flag[i]) continue;
			int v1=calcL1(p0, ar[i].st);
			int v2=calcL1(p0, ar[i].ed);
			int v=v1+v2;
			if (minv>v) {
				minv=v;
				mini=i;
	...
	static int minvst0(Point p0) {
		...
		for (int i=0; i<N; i++) {
			if (flag[i]) continue;
			int v=calcL1(p0, ar[i].st)+ar[i].length;
			if (minv>v) {
				minv=v;
				mini=i;
			}
	...
	static int minved0(Point p0) {
		...
		for (int i=0; i<N; i++) {
			if (flag[i]) continue;
			int v=calcL1(p0, ar[i].ed)+ar[i].length;
			if (minv>v) {
				minv=v;
				mini=i;
			}
	...
	static int minvsted(Point p0, Point pn) {
		...
		for (int i=0; i<N; i++) {
			if (flag[i]) continue;
			int v1=calcL1(p0, ar[i].st)+ar[i].length;
			if (v1>minv) continue;
			for (int j=0; j<N; j++) {
				if (flag[j]) continue;
				int v2=calcL1(pn, ar[j].ed)+ar[j].length;
				if (v1+v2>minv) continue;
				int v3=calcL1(ar[i].ed, ar[j].st);
				int v=v1+v2+v3;
				if (minv>v) {
					minv=v;
					mini1=i;
	...
	static class Rectangle {
		...
		Rectangle(Point st, Point ed) {
			this.st=st;
			this.ed=ed;
			this.length=calcL1(st.y, st.x, ed.y, ed.x);
		}
	...
	static int calcL1(Point p1, Point p2) { //x1,y1からx2,y2までのマンハッタン距離
		return calcL1(p1.y, p1.x, p2.y, p2.x);
	}
