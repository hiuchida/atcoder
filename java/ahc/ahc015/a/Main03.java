...
public class Main {
	...
	static int movef(int[][] ary, int y, int x) {
		int d=1;
		for (; true; d++) {
			int y2=y-d;
			if (y2<0 || ary[y2][x]!=0) break;
		}
		d--;
		return (y-d)*n+x;
	}
	static int moveb(int[][] ary, int y, int x) {
		int d=1;
		for (; true; d++) {
			int y2=y+d;
			if (y2>=n || ary[y2][x]!=0) break;
		}
		d--;
		return (y+d)*n+x;
	}
	static int movel(int[][] ary, int y, int x) {
		int d=1;
		for (; true; d++) {
			int x2=x-d;
			if (x2<0 || ary[y][x2]!=0) break;
		}
		d--;
		return y*n+x-d;
	}
	static int mover(int[][] ary, int y, int x) {
		int d=1;
		for (; true; d++) {
			int x2=x+d;
			if (x2>=n || ary[y][x2]!=0) break;
		}
		d--;
		return y*n+x+d;
	}
	static double score(int[][] ary, int nk) {
		...
		for (int y=0; y<n; y++) {
			for (int x=0; x<n; x++) {
				if (ary[y][x]==0) {
					int p1=movef(ary, y, x);
					int p2=moveb(ary, y, x);
					int p3=movel(ary, y, x);
					int p4=mover(ary, y, x);
					long v1=mp.get(p1);
					long v2=mp.get(p2);
					long v3=mp.get(p3);
					long v4=mp.get(p4);
					cnt++;
					sum+=Math.max(v1, Math.max(v2, Math.max(v3, v4)));
				}
			}
		}
