...
class DfsPartSolver {
 public:
  ...
  int score_ = 0;          // 今探索中のパスのスコア
  int remaining_search_cnt_;     // 残り探索回数
  int next_score_min_threhold_;  // 遷移に必要な閾値
  int goal_ = 0;                 // 目標地点
  ...
  void start(const int start, const int goal,
             ...
             const int next_score_min_threhold) {
    goal_ = goal;
    best_path_.clear();
    best_score_ = 0;
    score_ = 0;
    ...
    next_score_min_threhold_ = next_score_min_threhold;
    dfs(start);
  }
    ...
    if (coord != goal_) {
      ...
      for (const auto &next_coord : next_coords[coord]) {
        if (!visiteds_[tiles[next_coord]]) {
          ...
          // goalにたどり着いた時点で遷移可能なスコアになっていれば探索を終了
          if (score_ > next_score_min_threhold_) {
            best_score_ = score_;
            best_path_ = path_;
...
struct State {
  ...
  void simulatedAnnealingWithTimeThreshold(const int64_t time_threshold,
    ...
    for (;;) {
      ...
      // next_score - now_score>diff_thresholdから移項
      int next_score_min_threhold = now_score + diff_threshold;
      if (rev) {
        dfs_part_solver.start(now_path_[end_path_id], now_path_[start_path_id],
                              next_visiteds, remaining_search_cnt,
                              next_score_min_threhold);
      } else {
      ...
      // exp(diff/temp)>rnd.nextDouble()をlogをとって移行すると
      // diff > temp * rnd.nextLog()になる。
      // rnd.nextLog()は必ず負の値なので、diff>temp*rnd.nextLog()を満たせばdiff>0も満たす
      // この形なら閾値を先に決められるので近傍解を生成する途中で閾値と大小比較することで処理を短縮できる
      // 参考 https://qiita.com/not522/items/cd20b87157d15850d31c
      int next_score = dfs_part_solver.best_score_;
      int diff = next_score - now_score;
      if (dfs_part_solver.best_path_.size() > 0 && diff > diff_threshold) {
        // visitedsの更新
        ...
        if (rev) {
          for (int i = dfs_part_solver.best_path_.size() - 1; i >= 0; i--) {
