...
//			}
			for (int k=Math.min(1, lp.size()-1); k>=0; k--) {
				que.addLast(lp.get(k));
			}
			if (answer.score<lp.get(0).score) {
				answer=lp.get(0);
                if (!RELEASE) System.err.println("iteration: "+iteration+", score: "+answer.score+", len: "+answer.ans.length());
			}
			iteration++;
		}
		long end=System.currentTimeMillis();
...
//			return "score="+score;
			return "score="+score+",len="+ans.length();
