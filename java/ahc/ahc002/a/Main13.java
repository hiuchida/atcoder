...
//					Part part2=new Part(part.ans+m.d);
					Part part2=new Part(part.ans+m.d, part.score+m.score);
					if (part2.score>0) {
						lp.add(part2);
					}
				}
			...
			if (answer.score<lp.get(0).score) {
				answer=lp.get(0);
                if (!RELEASE) System.err.println("iteration: "+iteration+", score: "+answer.score+", que="+que.size());
			}
			iteration++;
		}
		long end=System.currentTimeMillis();
