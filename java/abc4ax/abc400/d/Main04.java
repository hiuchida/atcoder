...
//				break;
//			}
//			if (mz.map[p.y][p.x]!=0 && mz.map[p.y][p.x]<p.p) continue;
//			mz.map[p.y][p.x]=p.p;
			for (int d=0; d<DY.length; d++) {
				...
				int x2=x+DX[d];
				if (mz.check(y, x, p.p)) {
					que.addFirst(new Point(y, x, p.p));
					mz.map[p.y][p.x]=p.p;
				}
				if (mz.range(y, x) && mz.map[y][x]<0) {
					for (int dd=0; dd<DY.length; dd++) {
						if (mz.check(y+DY[dd], x+DX[dd], p.p+1)) {
							que.addLast(new Point(y+DY[dd], x+DX[dd], p.p+1));
							mz.map[y+DY[dd]][x+DX[dd]]=p.p+1;
						}
					}
				}
				if (mz.range(y2, x2) && mz.map[y2][x2]<0) {
					for (int dd=0; dd<DY.length; dd++) {
						if (mz.check(y2+DY[dd], x2+DX[dd], p.p+1)) {
							que.addLast(new Point(y2+DY[dd], x2+DX[dd], p.p+1));
							mz.map[y2+DY[dd]][x2+DX[dd]]=p.p+1;
						}
					}
				}
			}
...
//		mz.printmap();
	...
	static class Point implements Comparable<Point> {
		...
		public int compareTo(Point that) {
			int cmp = Integer.compare(this.p, that.p);
			if (cmp != 0) return cmp;
