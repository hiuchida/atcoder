...
//            	pr_r[r] = (pr_r[r] == 0) ? -999 : Math.log(pr_r[r]);
//            	pr_r[r] = Math.log(Math.max(pr_r[r], 1e-300));
            }
            double info = 0.0;
            for (int x = 0; x < pool.size(); x++) {
                int s = layoutVolumes[x];
...
//        		Cache c = cache[k][s];
//        		if (c != null)
//        			System.err.println("k="+k+" s="+s+" lb="+c.lb+" "+Arrays.toString(c.pr_if_x));
//        	}
//        }
    }
