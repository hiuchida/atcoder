...
// 油田1個分の状態
	...
	class State {
		...
		void move_to(int oil_id, int new_top_left) {
			...
			for (int q = 0; q < query_volumes.size(); ++q) {
				query_volumes.add(q, (byte) (oil_state.top_left_query_volumes.get(new_top_left, q)
	...
	class Query {
		...
		OilLayoutList pool; // 油田の状態についてのリスト

		Query(final Input input, OilLayoutList pool) {
...
// プールの確率を正規化する
	void normalize_pool(OilLayoutList pool) {
		double total = 0;
...
// 占いクエリを取得する
	IntList getDivinationQuery(final Input input, OilLayoutList pool, Sim sim) {
		...
		for (int _i = 0; _i < 3; ++_i) {
			...
			}
			{
				...
				while (add_k < no_info_coordinates.size()) {
					double eval = query.eval(sim, add_k + 1,
							(byte) (add_v + best_layout.volume.get(no_info_coordinates.get(add_k))));
					if (crt < eval) {
						crt = eval;
				...
				while (add_k > 0) {
					double eval = query.eval(sim, add_k - 1,
							(byte) (add_v - best_layout.volume.get(no_info_coordinates.get(add_k - 1))));
					if (crt < eval) {
						crt = eval;
	...
	void simulated_annealing(final Input input, final Sim sim, State state, OilLayoutList pool,
			LongDoubleMap hash_ln_lilelihood, int ITER) {
	...
	int main() {
		...
		OilLayoutList pool = new OilLayoutList();
		...
		for (int t = 0;; ++t) {
			...
			if (t == 0) {
				...
				simulated_annealing(input, sim, state, pool, hash_ln_lilelihood, ITER);
			}
			// この時点でpoolはソート済み
