...
// 占いクエリを取得する
	IntList getDivinationQuery(final Input input, Sim sim) {
	...
	void simulated_annealing(final Input input, final Sim sim, State state, final PairIntListListList swaps,
	...
	int main() {
		...
		for (int t = 0;; ++t) {
			...
			if (t == 0) {
				...
				simulated_annealing(input, sim, state, swaps, hash_ln_lilelihood, ITER);
			}
			// この時点でpoolはソート済み
			...
			if (best_pool_prob > 0.8) {
				...
				var query_coordinates = getDivinationQuery(input, sim);
				// 占いの評価値の方が高い場合、占いを行う
