...
public class Main {
    ...
    class BoolAry {
        ...
        BoolAry(int size, boolean def) {
            ...
            if (def != false) Arrays.fill(ary, def);
        }
    ...
    class DoubleAry {
        ...
        DoubleAry(int size, double def) {
            ...
            if (def != 0.0) Arrays.fill(ary, def);
        }
...
//    }
    ...
    class IntAryAry {
        ...
        IntAryAry(int size1, int size2, int def) {
            ...
            if (def != 0) {
                for (int i = 0; i < size1; i++) Arrays.fill(ary[i], def);
            }
        }
...
記事公開後、当日1位を超えたコード
00_complete.cppとの違い
- 難しさを判定し、難しくないときは5倍探索した(影響高)
- poolをグローバルに1個だけ置くようにした(影響低)
- push_backの代わりにemplace_backを使った(影響低)
*/
    ...
    for (int t = 0; t < ITER; ++t)
    {
        ...
        if (coin < slide_threshold)
        {
            ...
            if (i2 < input.n - oil.max_i && j2 < input.n - oil.max_j)
            {
                ...
                double next = hash_ln_lilelihood.count(state.hash) ? hash_ln_lilelihood.get(state.hash) : sim.ln_prob_state();
