    ...
    static void simulatedAnnealing(Input input, Sim sim, State state, List<OilLayout> pool, int[][][] swaps, Map<Long, Double> hashL, int iter, long startT) {
        ...
        for (int t = 0; t < iter; t++) {
            ...
            if (coin < 30) { // Slide neighbor
                ...
                int ni = bk1 / input.n + d[0], nj = bk1 % input.n + d[1];
                ...
                int ni1 = (bk2 / input.n) + (d1 / input.n), nj1 = (bk2 % input.n) + (d1 % input.n);
                int ni2 = (bk1 / input.n) + (d2 / input.n), nj2 = (bk1 % input.n) + (d2 % input.n);
            ...
            if (bestProb > 0.8) {
                ...
                for (int iter = 0; iter < 3; iter++) {
                    for (int i = 0; i < input.n2; i++) {
                        if (System.currentTimeMillis() - startT > 4000) throw new RuntimeException();
                        qe.flip(i);
