...
//        long[] mask = new long; // bitset<400>の代用（必要に応じて）
    ...
    public static void main(String[] args) {
        ...
        // M=2の全配置生成
        if (input.m == 2) {
            for (int i0 = 0; i0 <= input.n - input.oils[0].maxI - 1; i0++) {
                for (int j0 = 0; j0 <= input.n - input.oils[0].maxJ - 1; j0++) {
                    state.topLefts[0] = i0 * input.n + j0;
                    for (int i1 = 0; i1 <= input.n - input.oils[1].maxI - 1; i1++) {
                        for (int j1 = 0; j1 <= input.n - input.oils[1].maxJ - 1; j1++) {
                            state.topLefts[1] = i1 * input.n + j1;
