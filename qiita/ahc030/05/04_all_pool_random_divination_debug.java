...
//		}
    ...
    static void solve() {
            ...
            for (int jA = 0; jA < input.n - oilA.maxJ; jA++) {
                ...
                for (int iB = 0; iB < input.n - oilB.maxI; iB++) {
                    for (int jB = 0; jB < input.n - oilB.maxJ; jB++) {
                        ...
                        OilLayout l = new OilLayout(0.0, 0.0, state.topLefts, input.getVolume(state.topLefts));
                        l.id = genid++;
                        pool.add(l);
                    }
                }
        ...
        System.err.println(now()+" pool.size()="+pool.size()+" total="+input.total
        		+String.format(" max[0]=(%d,%d)", input.oils[0].maxI, input.oils[0].maxJ)
        		+String.format(" max[1]=(%d,%d)", input.oils[1].maxI, input.oils[1].maxJ));

        for (int t=0; true; t++) {
            if (System.currentTimeMillis() - startTime > 2900) sim.giveup(input, pool);
            if (t > 10) sim.giveup(input, pool);
            // 各配置の対数尤度を更新
            for (OilLayout l : pool) {
...
//                for (int ij = 0; ij < prob.length; ij++) {
//              	System.err.printf("%3.0f:", prob[ij]*100);
//              	System.err.printf("%3.0f ", calculateBinaryEntropy(prob[ij])*100);
//                	if (ij % input.n == input.n - 1) System.err.println();
//                }
            }
